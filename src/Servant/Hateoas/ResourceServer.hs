...
type family Resourcify api ct where
  ...
  Resourcify (Verb m s _ a)  ct = Verb m s '[ct] (Responsify ct a)
...
type family ResourcifyServer server ct m where
  ...
  ResourcifyServer (m a)       ct m = m (Responsify ct a)
...
instance {-# OVERLAPPABLE #-}
  ...
  , Replace2 server mkLink (m (Responsify ct a)) (IsFun mkLink) ~ ResourcifyServer server ct m
  ) => HasResourceServer (api :: Type) m ct where
  getResourceServer m _ api = pcomp2 (\(ma, self) -> buildResource (Proxy @ct) self <$> ma) (getHandler m api) mkSelf
    where
